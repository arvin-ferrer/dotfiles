...
-- opt.guicursor = ""
opt.guicursor = "i:ver30-iCursor-blinkwait300-blinkon200-blinkoff150"
...
-- Set color column at the 80th character

-- Dynamically update color column based on cursor position
vim.api.nvim_create_autocmd("CursorMoved", {
    pattern = "*",
    callback = function()
        local col = vim.fn.virtcol('.')  -- Get current cursor column
        vim.opt.colorcolumn = tostring(col)  -- Set color column at cursor
    end,
})
